...
<script type="text/javascript">
    ...
    // alert(uri);
    pesanan = <?php echo json_encode($pesanan); ?>;
    console.log(pesanan);
    // alert(pesanan);
    for (var no = 0; no <= pesanan.length)
        $('.link-' + no).addClass('visit');
    $(`<div class="row item itemNo-` + i + `" style="width:400px">
            <div class="col-md-6"><span for="" id="item-nama"> <a class="btn remove ` + no + `" id="remove` + i + `" style="margin-bottom='20px'"><i class="fas fa-minus-circle"></i></a>` + pesanan[i][0] + ` </span></div>
    ...
    $('.nav-menu').on('click', function() {
        ...
        $.get("<?php echo base_url() ?>Pemesanan/getDataPemesananById/" + id, {
